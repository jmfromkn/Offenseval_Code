...
np.random.seed(500)
threshold_gold = pd.read_csv("Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", sep='\t', header=0)
...
def store_data(train_path, test_path):
    ...
    for tweet in nlp.pipe(testlist, batch_size=1500, n_threads=8):
        ...
        dep = []
        ner = []
        ###Set up in format "TAG" and "Dep_Head_POS" and "Dep" for count vec or tfidf vec
        for word in tweet:
...
def predict(train_path, test_path, trainfeatures, testfeatures):
    ...
    prediction = svm_model(base_tables[0], base_tables[1], trainfeatures, testfeatures, 0.33, train_path)
    return prediction



stored_train = pd.read_csv("TrainFeaturesTaskB.csv", header=0)
stored_test = pd.read_csv("ThresholdGoldFeaturesTaskB.csv", header=0)

print ("Baseline: Only Embeddings")
full_train_baseline = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, 0:399], stored_test.iloc[:, 0:399])
print ("Only add NER")
full_train_NER = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:399, 1156:1177]], stored_test.iloc[:, np.r_[0:399, 1156:1177]])
print ("Only add POS")
full_train_POS = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:399, 1105:1155]], stored_test.iloc[:, np.r_[0:399, 1105:1155]])
print ("Only add LEX")
full_train_LEX = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:399, 400:407]], stored_test.iloc[:, np.r_[0:399, 400:407]])
print ("Only add DEP")
full_train_DEP = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:399, 408:1104]], stored_test.iloc[:, np.r_[0:399, 408:1104]])
print ("Syntactic Suite + Lex: POS + Dep + Lex, NO NER")
full_train_SYN_LEX = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, 0:1104], stored_test.iloc[:, 0:1104])
print ("Without POS Features")
full_train_no_POS = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:1104, 1156-1177]], stored_test.iloc[:, np.r_[0:1104, 1156-1177]])
print ("Without LEX features")
full_train_no_LEX = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:399, 408:1177]], stored_test.iloc[:, np.r_[0:399, 408:1177]])
print ("Without DEP Features")
full_train_no_DEP = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train.iloc[:, np.r_[0:407, 1105:1177]], stored_test.iloc[:, np.r_[0:407, 1105:1177]])
print ("Full Feature")
full_train = predict("Offenseval2020/task_b_distant/task_b_distant.tsv", "Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv", stored_train, stored_test)

testing = pd.read_csv("Offenseval2020/task_b_distant/ThresholdGold_TaskB.tsv",sep='\t', header=0)
...
testing.to_csv("dev_predictionsTaskB.csv", header=False, index=False)
