...
###If Task B, replace with path to Task B Development Set
...
def svm_model(train_data, test_data, train_features, test_features, threshold_value, path):
    ...
    return f1_score(predictions_SVM, Test_Y, average='macro')
...
def predict(train_path, test_path, trainfeatures, testfeatures):
    ...
    prediction = svm_model(base_tables[0], base_tables[1], trainfeatures, testfeatures, 0.41, train_path)
    return prediction


def simulate_thresholds(train_path, test_path, trainfeatures, testfeatures):
    base_tables = retrieve_table(train_path, test_path)
    best_i = 0
    best_f1 = 0
    start_value = 0.25
    ...
    top_i = []
    top_f1 = []
    for i in np.arange(start_value, max_value, 0.01):
        test_dif = svm_model(base_tables[0], base_tables[1], trainfeatures, testfeatures, i, train_path)
        top_i.append(i)
        top_f1.append(test_dif)
        if test_dif > best_f1:
            best_f1 = test_dif
            best_i = i
    print ("Best threshold value for sample: " + str(best_i))
    print ("Best threshold F1_score for that value: " + str(best_f1))
    return top_i, top_f1
###Replace with Task B Feature paths if testing on task B
stored_train = pd.read_csv("TrainFeatures.csv", header=0)
stored_test = pd.read_csv("ThresholdGoldFeatures.csv", header=0)

###Replace with Task B Train and Test path if testing on Task B
full_train = simulate_thresholds("Offenseval2020/task_a_distant/1MTweetRandomSample.tsv", "Offenseval2020/task_a_distant/ThresholdGold.tsv", stored_train, stored_test)

print (full_train[0])
print (full_train[1])

threshold_output = list(zip(full_train[0], full_train[1]))
output_df = pd.DataFrame(threshold_output, columnns=['Threshold Value', 'F1 Score'])
output_df.to_csv('ThresholdSimulationOutput.csv', header=0)